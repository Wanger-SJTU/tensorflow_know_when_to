...
class ShowAttendTell(object):
    ...
    def build_vgg16(self):
        ...
        if self.is_train:
            ...
            self.loss_op = 1e-5*tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
    ...
    def build_rnn(self):
        ...
        for idx in range(num_steps):
            ...
            with tf.variable_scope("lstm"):
                ...
                concat_input = tf.concat([context_mean, word_embed, last_output], 1)
                output, state = lstm(concat_input, last_state)
                memory, _ = state
            
            with tf.variable_scope("sentinel"):
                st = self.build_sentinal(concat_input, output, memory)
            
            with tf.variable_scope("attend"):
                alpha_t, beta_t, c_hat = self.attend_adaptive(contexts, output, st)
